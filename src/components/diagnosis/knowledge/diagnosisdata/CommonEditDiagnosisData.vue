<template>
  ...
  >
    <div class="dialogDiv">
      <el-form :model="editData" label-width="180px">
        ...
        <el-form-item label="诊断依据:(符号;换行)">
          <el-input type="textarea" v-model="editData.diagnosisBasis" />
        </el-form-item>
      </el-form>
    </div>
    <el-button
    ...
    <el-button type="info" @click="editDiagnosisDataClose()"> 取消 </el-button>
  </el-dialog>
</template>
<script>
export default {
  ...
  methods: {
    ...
    saveEditDiagnosisData() {
      this.putRequest("/modelPrinciple/modelPrinciple", this.editData).then(
        (resp) => {
          if (resp) {
            this.$message({
