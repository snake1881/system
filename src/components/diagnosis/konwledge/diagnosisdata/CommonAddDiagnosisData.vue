<template>
  ...
  >
    <div class="dialogDiv">
      <el-form :model="DiagnosisData" label-position="left" label-width="100px">
        ...
        <el-form-item label="诊断依据:">
          <el-input v-model="DiagnosisData.diagnosticBasis" />
        </el-form-item>
      </el-form>
    </div>
    <span slot="footer">
      <el-button type="primary" @click="saveAddDiagnosisData(), addDiagnosisDataClose()">
        提交
      </el-button>
    </span>
  </el-dialog>
...
export default {
  ...
  methods: {
    ...
    saveAddDiagnosisData() {
      this.postRequest("/knowledge/DiagnosticParametersGt/DiagnosticParametersGt", this.DiagnosisData).then(resp => {
        if (resp) {
          this.$message({
            message: "功图诊断参数新增成功!",
            type: "success"
